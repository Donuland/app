...
/* ========================================
   DONULAND SIMPLE CATEGORY FIX
   Používám kategorie přímo ze Sheets bez mapování
   ======================================== */

console.log('🔧 Donuland Simple Category Fix loading...');

// ========================================
// SIMPLE CATEGORY HANDLING
// ========================================

// Zjednodušená funkce - prostě vezme kategorii jak je
function normalizeCategory(category) {
    if (!category) return 'ostatní';
    
    // Prostě trim a lowercase/uppercase pro konzistenci
    const cleaned = category.trim();
    
    // Pokud je prázdné, dej ostatní
    if (!cleaned) return 'ostatní';
    
    // Jinak prostě vrať jak je
    return cleaned;
}
...
// Zjednodušené parsování - bez zbytečného mapování
function parseSheetData(csvText) {
    console.log('📝 Simple parsing CSV data...');
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        try {
            ...
            record.category = getColumnValue(values, 'F'); // PŘÍMĚ ZE SLOUPCE F
            ...
            // Debug kategorií
            if (i <= 5) {
                console.log(`Row ${i} - Category from F: "${record.category}"`);
            }
    ...
    // Debug - ukázat kategorie
    const categoryCount = {};
    data.forEach(record => {
        const cat = record.category || 'prázdné';
        categoryCount[cat] = (categoryCount[cat] || 0) + 1;
    });
    console.log('📊 Categories from Sheets:', categoryCount);
...
// Zjednodušené čištění dat
function validateAndCleanData(rawData) {
    console.log('🧹 Simple data cleaning...');
    ...
    for (const row of rawData) {
        try {
            ...
            const cleanRow = {
                ...
                category: normalizeCategory(row.category), // JEDNODUCHÉ čištění
    ...
    console.log(`✅ Cleaned data: ${cleanData.length} valid rows`);
    
    return cleanData;
}

// ========================================
// CATEGORY FACTORS (SIMPLIFIED)
// ========================================

// Zjednodušené kategorie faktory - jen základní
function getBaseCategoryFactor(category) {
    if (!category) return 0.10; // default
    
    const categoryLower = category.toLowerCase().trim();
    
    // Jednoduché mapování na faktory
    const factors = {
        'food festival': 0.15,
        'veletrh': 0.18,
        'koncert': 0.08,
    ...
    // Přímé mapování
    if (factors[categoryLower]) {
        return factors[categoryLower];
    }
    
    // Částečné shody
    if (categoryLower.includes('food') || categoryLower.includes('festival')) {
        return 0.15;
    }
    if (categoryLower.includes('veletrh')) {
        return 0.18;
    }
    if (categoryLower.includes('koncert') || categoryLower.includes('hudba')) {
        return 0.08;
    }
    if (categoryLower.includes('sport')) {
        return 0.10;
    }
    if (categoryLower.includes('kultur')) {
        return 0.12;
    }
...
// Override všech funkcí s jednoduchými verzemi
if (typeof window !== 'undefined') {
    ...
    window.getBaseCategoryFactor = getBaseCategoryFactor;
    
    console.log('🔧 All functions overridden with simplified versions');
}
...
// Funkce pro manuální aplikaci simple fix
window.applySimpleFix = function() {
    console.log('🔧 Applying simple category fix...');
    
    // Force reload data
    if (typeof loadData === 'function') {
        loadData().then(() => {
            setTimeout(() => {
                // Debug categories after load
                if (globalState.historicalData) {
                    console.log('📊 Data loaded, checking categories...');
                    
                    globalState.historicalData.slice(0, 5).forEach((record, i) => {
                    ...
                    if (typeof forceAnalyticsRefresh === 'function') {
    ...
    showNotification('🔧 Jednoduchá oprava aplikována - data se načítají...', 'info', 4000);
};
...
// Auto-aplikace při načtení dat
eventBus.on('dataLoaded', (data) => {
    console.log('🔧 Simple fix: Auto-applying after data load...');
    
    if (data.data && data.data.length > 0) {
        // Debug first few records
        console.log('📊 Sample data after simple fix:');
        data.data.slice(0, 3).forEach((record, i) => {
            console.log(`  ${i+1}. ${record.eventName} -> ${record.category} (${record.city})`);
        });
    }
});
...
// Jednoduchý debug pro kontrolu kategorií
window.debugSimpleCategories = function() {
    console.log('🔍 Simple category debug...');
    ...
    const categories = {};
    globalState.historicalData.forEach(record => {
        const cat = record.category || 'undefined';
    ...
    globalState.historicalData.slice(0, 5).forEach((record, i) => {
        console.log(`${i+1}. "${record.eventName}" -> "${record.category}" (${record.city}, ${record.sales} ks)`);
    });
};

console.log('✅ Simple Category Fix loaded');
console.log('🔧 Use: window.applySimpleFix() to reload data with simple categories');
console.log('🔍 Use: window.debugSimpleCategories() to debug categories');
console.log('📊 Categories are now taken directly from Sheets column F without complex mapping');
